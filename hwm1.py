...
while randomlist:
    ...
    min = randomlist[0]
    # Go through the random list and compare each number with vmin
    for x in randomlist:
        # If some element is less than min
        if x < min:
            min = x           # Variable min gets value of element
    sorted_list.append(min)   # Add min to sorted list
    randomlist.remove(min)    # Remove element from random list
