...
def more_setences(text_extended):
    ...
    for ext_sentense in range(len(text_extended)):
        if text_extended[ext_sentense].count(".") > 1:
            ...
            for i in ext_words:
                text_extended[ext_sentense] = text_extended[ext_sentense] + " " + i
        else:
            ext_word = text_extended[ext_sentense].split(" ")[-1]
            text_extended[ext_sentense] = text_extended[ext_sentense] + " " + ext_word
...
def text_misspell(text_misspelling):
    ...
    for misspel_sentense in range(len(text_misspelling)):
        ...
        text_misspelling[misspel_sentense] = re.sub(r'\Wiz\W', '"is"', text_misspelling[misspel_sentense])
...
if __name__ == "__main__":
    text_spliting = text_split(text)
    # print("Splited text\n", text_spliting)
    text_normalizing = text_normalize(text_spliting)
    # print("Normalized text\n", text_normalizing)
    extended_sentense = more_setences(text_normalizing)
    # print(extended_sentense)
    misspell_sentences = text_misspell(extended_sentense)
    # print(misspell_sentences)
    whitespaces = count_whitespaces(misspell_sentences)
